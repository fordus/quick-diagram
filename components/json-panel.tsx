...
export function JsonPanel({ diagramData, onImport, onClose }: JsonPanelProps) {
  ...
  const handleLoadExample = () => {
    const jsonString = JSON.stringify(DEFAULT_EXAMPLE, null, 2)
    setJsonInput(jsonString)
  }
  ...
  return (
    <Card className="h-full rounded-none border-0 flex flex-col">
      ...
      <CardContent className="p-4 space-y-3 flex-1 overflow-y-auto min-h-0">
        <Button onClick={handleImport} className="w-full gap-2 h-9 text-sm" disabled={!jsonInput.trim()}>
          <Upload className="h-3.5 w-3.5" />
          Import JSON
        </Button>

        <div className="flex gap-2">
          <Button onClick={handleExport} variant="outline" size="sm" className="gap-1.5 h-8 text-xs flex-1">
